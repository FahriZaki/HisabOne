...
import '../datasource/rmfunding_datasource.dart';

class RmFundingPage extends StatefulWidget {
  const RmFundingPage({super.key});

  @override
  State<RmFundingPage> createState() => _RmFundingPageState();
}

class _RmFundingPageState extends State<RmFundingPage> {
  late RmFundingDataSource _rmDataSource;
  List<RmFundingActivity> _activities = [];
  bool _loading = true;
  int _level = 4;
        ...
        if (jsonResponse["status"] == "success") {
          final data =
              (jsonResponse["data"] as List)
                  .map((e) => RmFundingActivity.fromJson(e))
                  .toList();
          ...
          setState(() {
            _activities = data;
            _rmDataSource = RmFundingDataSource(_activities, visibleColumns);
            _loading = false;
          });
        } else {
  ...
  List<GridColumn> _buildColumns() {
    final allColumns = [
      ...
      //  MTD NTB
      _col('MTD_NTB', 'MTD NTB', 120, Colors.purpleAccent),

      //  Fund Growth
      _col('FUND_GROWTH', 'Fund Growth', 130, Colors.redAccent),
    ];
