...
class _SnapshotPageState extends State<SnapshotPage> {
  ...
  Future<void> fetchDummyData() async {
    try {
      final url = Uri.parse(
        "http://103.59.95.71/api_performance/snapshot_fundingall.php",
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData["status"] == "success") {
          ...
          debugPrint("Funding All data kosong");
        }
      } else {
        setState(() => _isLoading = false);
        debugPrint("Gagal request Funding All: ${response.statusCode}");
      }
    } catch (e) {
      setState(() => _isLoading = false);
      debugPrint("Error Funding All: $e");
    }
  }
...
// Header normal (untuk kolom biasa)
